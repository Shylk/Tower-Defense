...
void animation_monsters(dfd_t *dfd)
{
    set_clock(dfd);
...
void display_break_def_and_looking_direction(dfd_t *dfd)
{
...
void change_wave_and_end_game(dfd_t *dfd)
{
...
void move_ennemies_level_2(dfd_t *dfd)
{
    animation_monsters(dfd);
    for (int i = 0; i < 13 + (dfd->lvl->wave * 2); i++) {
        dfd->lvl_1->current->freeze == 0 ? move_path_mob_lvl_2(dfd) : 0;
        dfd->lvl_1->current->alive == 0 ? sfRenderWindow_drawSprite
            (dfd->window, dfd->lvl_1->current->spr_mob, NULL) : 0;
        display_break_def_and_looking_direction(dfd);
...
void display_level_2(dfd_t *dfd)
{
    ...
    sfRenderWindow_drawSprite(dfd->window, dfd->lvl_2->spr_lvl_2_bg, NULL);
    if (dfd->end->end == 0) {
        if (dfd->escp->pause == 0) {
            move_ennemies_level_2(dfd);
            display_hud(dfd);
            launch_spell(dfd);
            tower_hit(dfd);
        }
